...
namespace controller {
    ...
    int
    start_module(thread_pool *pool, const std::string &name) {
    ...
    harvest_module(const std::string& module_name,
                    const int& cloud_id, const int& client_last_op) {
        //TODO(Célian) : open the database manager using the module_name
        DatabaseManager db_manager("fcloudpopulation");
        FPoint3DModel point_model(&db_manager);
        std::vector<FPoint3D> points = point_model.getPointListByCloudId(cloud_id);
        std::vector<Operation> operations;
        // TODO(Célian): refactor this ugly code an unit test it oh yeah
        for (FPoint3D point : points) {
            std::vector<int> op_indices = point.getOperationsIds();
            int point_last_op = op_indices.back();
            if (point_last_op <= client_last_op) {
                break;
            }
            int i1 = op_indices.size() - 1;
            int i2 = 0;
