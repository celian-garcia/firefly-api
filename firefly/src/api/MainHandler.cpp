...
#include "firefly_modules/fly/api/FlyHandler.hpp"
#endif
#ifdef WITH_CV_MODULE
#include "firefly_modules/cv/api/CvHandler.hpp"
#endif
        ...
        this->m_server->resource["^/api/(\\w+)/(\\w+)(\\?.*)?$"]["GET"] = [this](std::shared_ptr<HttpResponse> response,
            ...
            try {
                ...
                switch (process.getModuleType()) {
                    case ModuleType::FLY: {
#ifdef WITH_FLY_MODULE
                        handler = new fly_module::FlyHandler(process, processAction, queryParameters, this->m_pool);
#else
                        throw FireflyException(HtmlStatusCode::INTERNAL_SERVER_ERROR, "Fly Module not compiled.");
#endif
                        break;
                    }
                    case ModuleType::CV: {
#ifdef WITH_CV_MODULE
                        handler = new cv_module::FlyHandler(process, processAction, queryParameters, this->m_pool);
#else
                        throw FireflyException(HtmlStatusCode::INTERNAL_SERVER_ERROR, "Fly Module not compiled.");
#endif
                        break;
