...
struct adl_serializer<firefly::OperationType> {
    static void to_json(json &j, const firefly::OperationType &operation_type) {
        assert(operation_type == firefly::OperationType::ADD || operation_type == firefly::OperationType::DELETE);
